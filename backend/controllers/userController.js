...
const userController = {
  register: async (req, res) => {
    try {
      ...
      if (!name || !email || (provider !== 'google' && !password))
      ...
      if (provider !== 'google' && password.length < 6)
        return res
          .status(400)
          .json({ msg: "Password must be at least 6 characters." });

      const salt = await bcrypt.genSalt();
      const hashPassword = provider === 'google' ? undefined : await bcrypt.hash(password, salt);

      const newUser = { name, email, password: hashPassword, provider };

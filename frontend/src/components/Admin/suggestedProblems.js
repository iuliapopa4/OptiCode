...
const SuggestedProblems = () => {
  // State variables to store suggested problems and filter
  const [suggestedProblems, setSuggestedProblems] = useState([]);
  const [filter, setFilter] = useState('all');
  const { token } = useContext(AuthContext);

  // useEffect to fetch suggested problems when the component mounts and token changes
  useEffect(() => {
    if (!token) return;
  ...
  // Handle approving a suggested problem
  ...
  // Handle rejecting a suggested problem
